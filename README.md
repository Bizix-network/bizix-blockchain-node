...
Substrate](https://github.com/paritytech/polkadot-sdk/tree/master/substrate/bin/bizix-node)
upstream
...
```sh
./target/release/bizix-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/bizix-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/bizix-node -ldebug --dev
```
...
$ ./target/release/bizix-node --dev --base-path ./my-chain-state/
...
`pallets`](./pallets/bizix/src/lib.rs) directory.
